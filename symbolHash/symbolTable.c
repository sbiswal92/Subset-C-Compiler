...
struct symbolTable* symbolTable_addChild(struct symbolTable* symbolTable) {
	...
	childSymbolTable->parentSymbolTableEntrySymbol = symbolTable->symbolListHead->symbol;
    childSymbolTable->parentSymbolTable = symbolTable;
    return childSymbolTable;
}

struct symbolTable* symbolTable_getChild(struct symbolTable* symbolTable, struct symbol *parentEntrySymbol) {
	//iterate through children of this symbolTable
	struct symbolTableListEntry *currChildTable=symbolTable->childSymbolTableListHead;
	//Check for the child symbolTable whose parentSymbolTableEntry->symbol matches parentEntrySymbol
	while(currChildTable->parentSymbolTableEntrySymbol->symbol!=parentEntrySymbol)
	{
		currChildTable = currChildTable->nextEntry;
	}
